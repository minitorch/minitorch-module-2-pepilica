...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return a.f.add_zip(
            a.f.mul_zip(sigmoid, grad_output),
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        # алиса включи песню из фильма где мужик сигма и еще он с барменшей ругается и там клуб короче
        new_order = list(int(order[i]) for i in range(order.size))
        ctx.save_for_backward(new_order)
        permuted_a = a._tensor.permute(*new_order)
        return minitorch.Tensor.make(
            permuted_a._storage, permuted_a.shape, permuted_a.strides, a.backend
        )
